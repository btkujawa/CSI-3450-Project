...
if (array_key_exists('idUsers', $_POST) && array_key_exists('username', $_POST) && array_key_exists('password', $_POST)) {
...
elseif (array_key_exists('idUsers', $_POST) && array_key_exists('username', $_POST)) {
    $idUsers = $_POST['idUsers'];
    $username = $_POST['username'];
    $sql = "UPDATE Users SET username = '$username' WHERE idUsers = '$idUsers';";
    mysqli_query($conn, $sql);
}
elseif (array_key_exists('idUsers', $_POST) && array_key_exists('password', $_POST)) {
    $idUsers = $_POST['idUsers'];
    $password = $_POST['password'];
    $password = hash('sha256', $password, false);
